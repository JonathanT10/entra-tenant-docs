...
- **Policy names are kept — with the obvious hole closed.** Conditional Access and Intune policy names stay as written, so the gap analysis stays readable — but any name the anonymizer already replaced elsewhere (a person, group, app, named location, domain, or the organization itself) is substituted *inside* them too: `CA - Restrict relay to HQ Offices` becomes `CA - Restrict relay to Location a1b2c3`. Matching is case-insensitive and whole-token, and names under 4 characters are left alone so a group called `IT` can't mangle every policy name containing those letters. What this cannot catch is free text that identifies you without naming a mapped object — read the names before sharing, or use `-AnonymizePolicyNames` to pseudonymize them entirely (the gap analysis will then read `satisfied by 'Policy 2c9f'`, which is the price).
...
- **`-Anonymize` is not a guarantee.** It replaces the categories listed above, and it substitutes known directory names inside the Conditional Access and Intune policy names it keeps. What remains is free text an admin wrote, and free text is where surprises live — a policy name can identify you without naming any directory object. Read the output before you share it; don't treat the switch as permission to skip that.
